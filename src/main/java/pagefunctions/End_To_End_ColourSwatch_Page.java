...
public class End_To_End_ColourSwatch_Page {
	...
	public void selectColourFromSwatch(String colourName) {
		...
		// Colour card
		By colourCard = By
				.xpath("//div[contains(@class,'colour-swatch-card') and @data-colorname='" + colourName + "']");
	...
	public void selectQuantity(int quantity) {
		...
		// wait quantity
		WebElement qtyActionDiv = wait
				.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".cmp-product__actions--mini")));

		// increment button & quantity span
		WebElement incrementBtn = qtyActionDiv.findElement(By.cssSelector(".cmp-product__quantity--increment"));

		WebElement qtySpan = qtyActionDiv.findElement(By.cssSelector(".cmp-product__quantity"));
		...
		for (int i = currentQty; i < quantity; i++) {
			...
			js.executeScript("arguments[0].dispatchEvent(new Event('input',{bubbles:true}));", qtySpan);
			js.executeScript("arguments[0].dispatchEvent(new Event('change',{bubbles:true}));", qtySpan);

			int expectedQty = i + 1;
			wait.until(d -> Integer.parseInt(qtySpan.getText().trim()) == expectedQty);
		}
	...
	public void clickWishlistIcon() {
		...
		// 1️ Wait for initial loader to disappear
		wait.until(ExpectedConditions.invisibilityOfElementLocated(loader));

		// 2️ Locate wishlist icon
	...
	public void clickTabFromList(String tabName) {
		...
		List<WebElement> tabs = driver.findElements(By.xpath("//ol[@role='tablist']//li[@role='tab']"));
	...
	public void selectOptionFromWishlist(String optionText) {
		...
		WebElement optionBtn = wait.until(ExpectedConditions
				.presenceOfElementLocated(By.xpath("(//button[contains(@class,'cmp-product__option-button')])[1]")));
		...
		wait.until(driver -> driver.findElements(By.xpath("//ul[contains(@class,'cmp-product__options')]")).size() > 0);

		// Step 4: Now click option directly (no need to wait for full visibility)
		WebElement option = wait.until(ExpectedConditions
				.presenceOfElementLocated(By.xpath("//span[normalize-space()='" + optionText + "']")));
	...
	public boolean isProductRemovedFromWishlist() {
		...
		return wait.until(driver -> driver
				.findElements(By.xpath("//div[contains(@class,'profile-swatch__info-swatch')]")).isEmpty());
	}
}
