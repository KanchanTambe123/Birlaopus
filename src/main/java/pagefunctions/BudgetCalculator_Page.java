...
public class BudgetCalculator_Page {
	...
	public void selectSpaceOptions(String spaceName) {
	    ...
	    List<WebElement> spaceOptions = wait.until(
	...
	public void verifyTabIsLocked(String tabName) {
	    ...
	    WebElement tab = wait.until(ExpectedConditions.presenceOfElementLocated(
	            By.xpath("//li[@role='tab' and normalize-space()='" + tabName + "']")
	    ));

	    JavascriptExecutor js = (JavascriptExecutor) driver;

	    // Read CSS custom property
	    String lockBg = (String) js.executeScript(
	            "return window.getComputedStyle(arguments[0]).getPropertyValue('--before-bg');",
	            tab
	    );

	    Assert.assertTrue(
	            lockBg != null && lockBg.contains("lockForTabs"),
	...
	public void verifyButtonNotVisibleOnPremiumTab() {
	    ...
	    List<WebElement> buttons = driver.findElements(
	        By.xpath("//button[.//span[normalize-space()='Book a free survey']]")
	    );
	...
	public void verifyButtonVisibilityOnTab(String tabName) throws InterruptedException {
	    ...
	    By tabLocator = By.xpath(
	        "(//li[@role='tab' and normalize-space()='" + tabName + "'])[2]"
	    );

	    WebElement tab = wait.until(
	        ExpectedConditions.presenceOfElementLocated(tabLocator)
	    );
	    ...
	    By bookSurveyBtn = By.xpath(
	        ".//a[normalize-space()='Book a free survey'] | " +
	    ...
	    Assert.assertTrue(
	        button.isDisplayed(),
	        "'Book a free survey' button should be visible on " + tabName + " tab"
	    );
	}
	...
	public void closeThankYouPopup() {
	    ...
	    ((JavascriptExecutor) driver).executeScript(
	        "arguments[0].scrollIntoView(true);", closeBtn
	    );
