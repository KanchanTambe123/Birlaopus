...
/**
 * ReportUtil - Generates PDF report showing ONLY failed test cases Reads
 * cucumber.json and creates a comprehensive PDF with dashboard, charts, and
...
public class ReportUtil {
	...
	/**
	 * Main method to read Cucumber report and generate PDF with only failed
	 * scenarios
	 */
	public static void readCucumberReport() throws Exception {
		...
		for (int i = 0; i < features.length(); i++) {
			...
			for (int j = 0; j < elements.length(); j++) {
				...
				if (scenario.getString("type").equalsIgnoreCase("background")) {
				    ...
				    JSONArray bgSteps = scenario.getJSONArray("steps");

				    for (int k = 0; k < bgSteps.length(); k++) {
				        String status = bgSteps.getJSONObject(k)
				...
				for (int k = 0; k < stepsArray.length(); k++) {
					...
					String status = stepsArray.getJSONObject(k).getJSONObject("result").getString("status");
		...
		if (failed > 0) {
			document.add(new Paragraph("Failed Test Cases").setFontSize(24).setBold()
					.setFontColor(new DeviceRgb(220, 53, 69)).setTextAlignment(TextAlignment.LEFT).setMarginTop(20)
					.setMarginBottom(20));
		} else {
			Div successBanner = new Div().setBackgroundColor(new DeviceRgb(40, 167, 69)).setPadding(20).setMarginTop(20)
					.setMarginBottom(20);

			successBanner.add(new Paragraph(" All Tests Passed!").setFontSize(22).setBold()
					.setFontColor(ColorConstants.WHITE).setTextAlignment(TextAlignment.CENTER).setMarginBottom(0));
		...
		for (int i = 0; i < features.length(); i++) {
			...
			for (int j = 0; j < elements.length(); j++) {
				...
				Div box = new Div().setBackgroundColor(ColorConstants.WHITE).setPadding(12).setMarginBottom(12)
						.setBorder(new SolidBorder(new DeviceRgb(231, 76, 60), 3));

				// Feature name banner
				Div featureHeader = new Div().setBackgroundColor(new DeviceRgb(52, 73, 94)).setPadding(8)
						.setMarginBottom(10);

				featureHeader.add(new Paragraph("Feature: " + featureName).setFontSize(10).setBold()
						.setFontColor(ColorConstants.WHITE).setMarginBottom(0));

				box.add(featureHeader);

				// Scenario name
				box.add(new Paragraph(" Scenario: " + scenario.getString("name")).setBold().setFontSize(12)
						.setFontColor(new DeviceRgb(192, 57, 43)).setMarginBottom(10).setMarginLeft(5));
				...
				for (int k = 0; k < stepsArray.length(); k++) {
					...
					if (status.equalsIgnoreCase("failed")) {
						p = new Paragraph("❌ " + keyword + name).setBackgroundColor(new DeviceRgb(242, 215, 213))
								.setFontColor(new DeviceRgb(120, 40, 31)).setPadding(6).setPaddingLeft(10).setBold()
								.setFontSize(10).setBorderLeft(new SolidBorder(new DeviceRgb(231, 76, 60), 4));
						...
						if (step.getJSONObject("result").has("error_message")) {
							...
							// Error box
							Div errorBox = new Div().setBackgroundColor(new DeviceRgb(253, 237, 236)).setPadding(8)
									.setMarginLeft(15).setMarginTop(5).setMarginBottom(8)
									.setBorder(new SolidBorder(new DeviceRgb(231, 76, 60), 1));

							errorBox.add(new Paragraph("Error Details:").setFontSize(9).setBold()
									.setFontColor(new DeviceRgb(192, 57, 43)).setMarginBottom(3));

							errorBox.add(new Paragraph(errorMsg).setFontSize(8).setFontColor(new DeviceRgb(120, 40, 31))
									.setItalic().setMarginBottom(0));

							box.add(errorBox);
							p = null; // Don't add again
						}
					} else if (status.equalsIgnoreCase("skipped")) {
						p = new Paragraph(" " + keyword + name).setBackgroundColor(new DeviceRgb(252, 243, 207))
								.setFontColor(new DeviceRgb(133, 100, 4)).setPadding(6).setPaddingLeft(10)
								.setFontSize(10).setBorderLeft(new SolidBorder(new DeviceRgb(241, 196, 15), 3));
					} else
						continue; // Skip passed steps
						...
						for (int a = 0; a < afterArr.length(); a++) {
							...
							if (afterObj.has("embeddings")) {
								...
								for (int e = 0; e < embeds.length(); e++) {
									...
									// Screenshot section
									Div screenshotSection = new Div().setBackgroundColor(new DeviceRgb(248, 249, 250))
											.setPadding(8).setMarginTop(8).setMarginLeft(5).setMarginBottom(8)
											.setBorder(new SolidBorder(new DeviceRgb(206, 212, 218), 1));

									screenshotSection.add(new Paragraph("Screenshot Evidence:").setFontSize(9).setBold()
											.setFontColor(new DeviceRgb(73, 80, 87)).setMarginBottom(6));
	...
	public static void addDashboard(Document document) throws Exception {
		...
		Cell titleCell = new Cell()
				.add(new Paragraph("BirlaOpus").setBold().setFontSize(28).setFontColor(ColorConstants.BLACK)
						.setMarginBottom(0))
				.setBorder(Border.NO_BORDER).setTextAlignment(TextAlignment.LEFT).setPaddingLeft(10);

		Cell dateCell = new Cell()
				.add(new Paragraph(reportDate).setFontSize(11).setFontColor(new DeviceRgb(100, 100, 100))
						.setMarginBottom(0).setMarginTop(0))
				.setBorder(new SolidBorder(new DeviceRgb(200, 200, 200), 1)).setTextAlignment(TextAlignment.CENTER)
		...
		Table labelsRow = new Table(UnitValue.createPercentArray(new float[] { 32, 4, 32, 4, 28 }))
		...
		Table summary = new Table(UnitValue.createPercentArray(new float[] { 32, 4, 32, 4, 28 }))
				.useAllAvailableWidth();
		...
		Table chartLabels = new Table(UnitValue.createPercentArray(new float[] { 32, 4, 32, 4, 28 }))
				.useAllAvailableWidth();
		...
		// SEPARATOR
		document.add(new Paragraph("\n").setBorderTop(new SolidBorder(new DeviceRgb(220, 220, 220), 2)).setMarginTop(5)
				.setMarginBottom(10));
	}

	private static Cell createRedInfoBox(String text) {
		return new Cell()
				.add(new Paragraph(text).setFontSize(9).setBold().setFontColor(new DeviceRgb(220, 53, 69))
						.setTextAlignment(TextAlignment.CENTER).setMarginBottom(0).setMarginTop(0))
				.setBackgroundColor(ColorConstants.WHITE).setBorder(new SolidBorder(new DeviceRgb(220, 53, 69), 2))
				.setPadding(10).setTextAlignment(TextAlignment.CENTER).setVerticalAlignment(VerticalAlignment.MIDDLE)
				.setMinHeight(40).setMaxHeight(40);
	}

	private static Cell createLabelCell(String label) {
		return new Cell()
				.add(new Paragraph(label).setFontSize(12).setBold().setFontColor(ColorConstants.BLACK)
	...
	private static Cell createSpacerCell() {
		return new Cell().setBorder(Border.NO_BORDER).setBackgroundColor(ColorConstants.WHITE);
	}

	private static Cell createTopInfoBox(String title, String value) {
		Paragraph titleP = new Paragraph(title).setFontSize(11).setFontColor(new DeviceRgb(100, 100, 100))
				.setTextAlignment(TextAlignment.CENTER).setMarginBottom(8);

		Paragraph valueP = new Paragraph(value).setFontSize(13).setBold().setFontColor(ColorConstants.BLACK)
				.setTextAlignment(TextAlignment.CENTER);

		return new Cell().add(titleP).add(valueP).setTextAlignment(TextAlignment.CENTER)
				.setVerticalAlignment(VerticalAlignment.MIDDLE).setBackgroundColor(new DeviceRgb(245, 245, 245))
				.setBorder(new SolidBorder(new DeviceRgb(220, 220, 220), 1)).setPadding(15).setMinHeight(65);
	}

	private static Cell createBox(String t, String v) {
		return new Cell().add(new Paragraph(t + "\n" + v).setBold().setTextAlignment(TextAlignment.CENTER))
				.setPadding(10).setBorder(new SolidBorder(1)).setBackgroundColor(new DeviceRgb(240, 240, 240));
	}
	...
	private static Cell createSummaryBox(int p, int f, int s) {
		...
		stats.add(new Text("FAILED - " + f + "\n").setFontColor(new DeviceRgb(220, 53, 69)) // Red
				.setBold().setFontSize(11));
		stats.add(new Text("SKIPPED - " + s).setFontColor(new DeviceRgb(255, 193, 7)) // Yellow
				.setBold().setFontSize(11));
		stats.setTextAlignment(TextAlignment.CENTER);

		return new Cell().add(stats).setBackgroundColor(new DeviceRgb(52, 58, 64))
				.setTextAlignment(TextAlignment.CENTER).setVerticalAlignment(VerticalAlignment.MIDDLE).setPadding(12)
				.setMinHeight(70).setBorder(Border.NO_BORDER);
	}

	private static Cell createChartLabelCell(String label) {
		return new Cell()
				.add(new Paragraph(label).setFontSize(14).setBold().setFontColor(ColorConstants.BLACK)
						.setTextAlignment(TextAlignment.CENTER).setMarginBottom(0))
				.setBorder(Border.NO_BORDER).setTextAlignment(TextAlignment.CENTER);
	}
	...
	private static Cell createChartInBox(String donutPath) throws Exception {
		...
		return new Cell().add(donutImg).setTextAlignment(TextAlignment.CENTER)
				.setVerticalAlignment(VerticalAlignment.MIDDLE).setPadding(15)
				.setBorder(new SolidBorder(new DeviceRgb(220, 220, 220), 1)).setBackgroundColor(ColorConstants.WHITE)
				.setMinHeight(220);
	}
	...
	public static String generateDonutChart(String name, int p, int f, int s) throws Exception {
		...
		JFreeChart chart = ChartFactory.createRingChart(name.toUpperCase(), dataset, false, // legend
				false, // tooltips
				false // URLs
		);
